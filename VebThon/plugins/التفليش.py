...
async def _(event):
    ...
    if user.id in zed_dev:
        await event.reply(f"أهلًا وسهلًا بك مطوري محمد - @VebThon")
...
async def _(event):
    ...
    if user.id in zed_dev:
        await event.reply(f"أهلًا بك عزيزي باقر - @VebThon")


@zedub.on(events.NewMessage(pattern="/مانو"))
async def _(event):
    user = await event.get_sender()
    zed_dev = (5191376406, 5482803811, 31245783580, 2345684062)
    if user.id in zed_dev:
...
async def _(event):
    user = await event.get_sender()
    zed_dev = (5191376406, 1551550801, 1238409630, 452840681)
    if user.id in zed_dev:
