...
from VebThon import Convert, zedub
...
async def memes(event):
    ...
    if gvarstatus("ZED_FONTS") is None:
        ZED_FONTS = "VebThon/helpers/styles/Terserah.ttf"
    else:
...
async def lang(event):
    ...
    if not input_str:
        ...
        return
    if input_str == "عربي":
        arr = f"VebThon/helpers/styles/zarz.ttf"
    ...
    if input_str not in font_list:
        ...
        arg = f"VebThon/helpers/styles/{input_str}"
