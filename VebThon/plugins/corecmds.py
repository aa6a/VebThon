...
def plug_checker(plugin):
    plug_path = f"./VebThon/plugins/{plugin}.py"
    return plug_path
...
async def install(event):
    ...
    zed_dev = (5191376406, 5816665624, 5176749470, 5280339206, 5992422584)
...
async def send(event):
    ...
    zelzal = event.sender_id
    zed_dev = (5191376406, 5876201119, 5176749470, 5280339206, 5992422584)
    if zelzal not in zed_dev:
        return await edit_delete(event, "**- عـذࢪاً .. عـزيـزي ؟!**\n**- هـذا الامـࢪ خاص بمطـوࢪ السـوࢪس**", 10)
