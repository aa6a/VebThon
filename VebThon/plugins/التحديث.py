...
from VebThon import HEROKU_APP, UPSTREAM_REPO_URL, zedub
...
async def upstream(event):
    ...
    if conf == "الان":
        ...
        await event.edit("ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗩𝗘𝗕𝗧𝗛𝗢𝗡  - تحـديثـات السـورس\n**•─────────────────•**\n\n**⇜ يتـم تحـديث بـوت تيبــثون .. انتظـر . . .🌐**\n\n%𝟷𝟶 ▬▭▭▭▭▭▭▭▭▭")
        await asyncio.sleep(1)
        await event.edit("ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗩𝗘𝗕𝗧𝗛𝗢𝗡  - تحـديثـات السـورس\n**•─────────────────•**\n\n**⇜ يتـم تحـديث بـوت تيبــثون .. انتظـر . . .🌐**\n\n%𝟸𝟶 ▬▬▭▭▭▭▭▭▭▭")
        await asyncio.sleep(1)
        await event.edit("ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗩𝗘𝗕𝗧𝗛𝗢𝗡  - تحـديثـات السـورس\n**•─────────────────•**\n\n**⇜ يتـم تحـديث بـوت تيبــثون .. انتظـر . . .🌐**\n\n%𝟹𝟶 ▬▬▬▭▭▭▭▭▭▭")
...
async def upstream(event):
    ...
    off_repo = "https://github.com/VebThonee/nekopack"
    os.chdir("/app")
    try:
        txt = (
...
async def reda(event):
    ...
    if event.message.message == "تحديث اجباري" and event.sender_id in progs:
        ...
        try:
            ...
            origin.fetch()
            force_update = True
            repo.create_head("VebThon", origin.refs.VebThon)
            repo.heads.VebThon.set_tracking_branch(origin.refs.VebThon)
            repo.heads.VebThon.checkout(True)
        ac_br = repo.active_branch.name
        if ac_br != UPSTREAM_REPO_BRANCH:
            await event.edit(
                "**[UPDATER]:**\n"
        ...
        if conf == "" and not force_update:
            ...
            return await event.respond(
                f"⌔ :  لتحديث سورس فيبثـون ارسل : `.تحديث الان` "
            )
    ...
    if event.reply_to and event.sender_id in progs:
        ...
        if owner_id == zedub.uid:
            if event.message.message == "حدث":
                conf = "الان"
                event = await event.reply("**ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗩𝗘𝗕𝗧𝗛𝗢𝗡  - تحـديثـات السـورس\n**•─────────────────•**\n\n**✾╎جـاري تـحــديــث الـسـورس بأمــر مـن المـطور ⎌****")
                off_repo = UPSTREAM_REPO_URL
                force_update = False
