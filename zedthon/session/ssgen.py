...
def telethon_session():
    ...
    # logging in
    try:
        with TelegramClient(StringSession(), API_ID, API_HASH) as VebThon:
            print("Generating a string session for •VebThon•")
            try:
                VebThon.send_message(
                    "me",
                    f"**- هـذا هو كـود سيشـن تيليثـون الخـاص بـك ☑️**:\n\n`{VebThon.session.save()}`\n\n**- لا تشـارك هـذا الكـود مع اي احـد ⚠️؟!**\n**- فقـط قم بنسخـه ولصقـه في مربعـات التنصيب 🛃**\n\n**- سـورس تيبـثون : @VebThon**",
                )
                print(
...
def pyro_session():
    ...
    with Client(
        name="VebThon", api_id=API_ID, api_hash=API_HASH, in_memory=True
    ) as pyro:
